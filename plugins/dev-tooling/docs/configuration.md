...
> [!TIP]
> One repository setting removes the recurring worktree friction. `git config worktree.useRelativePaths true` (git 2.48 or newer) makes every later worktree carry relative linkage from the start — `git worktree add` otherwise writes an absolute pointer that needs the repair command before the first tool call. Pass an explicit starting point (`HEAD`, a branch, or a commit) so the worktree carries the code you mean to test; without one, `git worktree add` branches from the repository's default branch.
