...
- **A `beforeShellExecution` / `postToolUse` hook on `git worktree add` / `git worktree remove`.** `hooks/scripts/worktree-directives.sh` reminds the session to call `set_project_root` on all three servers after either command, since each server is a separate process holding its own sticky root. On enter it also reads the worktree's `.git` file when that path already exists and, when the pointer is absolute — which `git worktree add` writes unless the repository sets `worktree.useRelativePaths` — names the repair command up front, and it warns that a worktree created without an explicit starting point branches from the repository's default branch.
...
- **`dev-tooling-runner` could be steered into dependency installs.** The runner's documented tool list now excludes `worktree_prepare`, a `vendor/`/`node_modules`-rewriting setup command outside its mutation boundary — and the dependency refusals name that tool as the remedy. The runner reports such refusals instead of acting on them.
...
### Added
- **Cursor plugin sidecar.** `.cursor-plugin/plugin.json` points Cursor at `./.mcp.json` and `hooks/cursor-hooks.json` so the three MCP servers and the enforcement hooks install from a Cursor marketplace without a second plugin tree. Hook scripts accept Cursor `beforeShellExecution` payloads (`command`, `CURSOR_PROJECT_DIR`, `workspace_roots`) and emit both Claude `additionalContext` and Cursor `additional_context`. phpactor via `.lsp.json` remains Claude Code only.
