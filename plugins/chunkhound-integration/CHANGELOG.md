...
### Changed
- **Cursor-only.** Removed Claude Code packaging. MCP lives in `mcp.json` with `${CURSOR_PLUGIN_ROOT}`. Session hook emits only `additional_context`. Skills and the session prompt use short MCP names (`code_research`, `search`, `daemon_status`).
