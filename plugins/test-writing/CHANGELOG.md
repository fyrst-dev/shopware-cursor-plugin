...
### Changed
- **Cursor-only.** Removed Claude Code packaging. MCP lives in `mcp.json` with `${CURSOR_PLUGIN_ROOT}`. Rule packages write to `CURSOR_PLUGIN_DATA` or `<workspace>/.cursor/test-writing`.
- Team review is a Cursor `Task` campaign. Bash gates moved to `skills/phpunit-test-team-reviewing/scripts/`.

### Removed
- Claude Workflow runtime (`workflow/team-review.workflow.mjs`, `build-run-script.sh`).
