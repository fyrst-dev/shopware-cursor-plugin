...
### Changed
- **Cursor-only.** Removed the Claude Code `plugin.json`. Skills call ChunkHound via short MCP names (`code_research`). Claude Agent SDK embedding is not part of this plugin.

## [1.1.0] - 2026-09-15

### Added
- **Cursor plugin sidecar.** `.cursor-plugin/plugin.json` so the PR and issue analysis skills install from a Cursor marketplace. Claude Agent SDK embedding remains Claude-only; the skills still run in Cursor chat.
