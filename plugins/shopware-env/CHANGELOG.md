...
### Changed
- **Cursor-only.** Removed Claude Code packaging. MCP lives in `mcp.json` with `${CURSOR_PLUGIN_ROOT}`. Hooks use Cursor events and emit only `additional_context`. Lifecycle skill uses short MCP names.

## [1.4.0] - 2026-09-15

### Added
- **Cursor plugin sidecar.** `.cursor-plugin/plugin.json` points Cursor at `./.mcp.json` and `hooks/cursor-hooks.json`. Lifecycle enforcement hooks accept Cursor `beforeShellExecution` payloads and SessionStart emits both Claude and Cursor context fields.
