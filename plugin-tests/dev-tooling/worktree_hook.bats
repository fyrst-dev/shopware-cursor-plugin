...
# Runs the hook and replaces $output with the decoded additional_context string.
...
_run_hook_and_decode_context() {
    local payload="$1"
    run bash -c 'printf "%s" "$1" | bash "$2" | jq -r ".additional_context"' \
...
# Cursor reads the directive out of additional_context. A hook that keeps
...
@test "the hook emits a Cursor additional_context envelope holding the directive as a string" {
    run bash -c 'printf "%s" "$1" | bash "$2" | jq -r "[
            (.additional_context | type),
            (.additional_context | startswith(\"Worktree entered: \") | tostring)
        ] | join(\"|\")"' \
        _ "$(_enter_input "/repo/.claude/worktrees/feature" "/repo/.claude/worktrees/feature")" \
        "${HOOK_SCRIPT}"
    assert_success
    assert_output "string|true"
}
...
@test "EnterWorktree with a worktree path warns that the default branch is not necessarily HEAD" {
    _run_hook_and_decode_context "$(_enter_input "/repo/.claude/worktrees/feature" "/repo/.claude/worktrees/feature")"
    assert_success
    assert_output --partial "branches from the repository's default branch"
}
...
# --- Cursor beforeShellExecution / Shell payloads ---

_cursor_command() {
    jq -cn --arg cmd "$1" '{command: $cmd}'
}

@test "git worktree add names the path and every server" {
    _run_hook_and_decode_context "$(_cursor_command "git worktree add /repo/.worktrees/feature")"
    assert_success
    assert_output --partial "Worktree entered: /repo/.worktrees/feature"
    assert_output --partial 'set_project_root with project_root "/repo/.worktrees/feature"'
    assert_output --partial "php-tooling"
    assert_output --partial "js-admin-tooling"
    assert_output --partial "js-storefront-tooling"
}

@test "git worktree add -b skips the branch operand and names the path" {
    _run_hook_and_decode_context "$(_cursor_command "git worktree add -b feature /repo/.worktrees/feature")"
    assert_success
    assert_output --partial "Worktree entered: /repo/.worktrees/feature"
}

@test "git worktree remove names every server and the no-argument call" {
    _run_hook_and_decode_context "$(_cursor_command "git worktree remove /repo/.worktrees/feature")"
    assert_success
    assert_output --partial "Call set_project_root with no project_root argument"
    assert_output --partial "php-tooling"
}

@test "an unrelated shell command produces no output" {
    _run_worktree_hook "$(_cursor_command "git status")"
    assert_success
    assert_output ""
}
